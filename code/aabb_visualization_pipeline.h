void create_aabb_visualization_pipeline(pipeline_info* pipeline)
{
    int error;
    ...
    //pipeline
    VkShaderModule vert_shader = create_shader_module_from_file("data/shaders/aabb_visualization_vert.spv");
    VkShaderModule frag_shader = create_shader_module_from_file("data/shaders/aabb_visualization_frag.spv");
...
pipeline_info* aabb_visualization_pipeline = add_pipeline(create_aabb_visualization_pipeline);
...
void draw_aabb()
{
    ...
    vkCmdBindPipeline(vkon.command_buffers[f], VK_PIPELINE_BIND_POINT_GRAPHICS, aabb_visualization_pipeline->pipeline);
    vkCmdDraw(vkon.command_buffers[f], 24, ssbo->n_nodes, 0, 0);
}
#endif
