...
int end_shader()
{
    ...
    *shader_source++ = 0;
    ...
    if(error == GL_FALSE)
    {
        ...
        *d++ = 0;
        log_warning("could not compile shader ", program_name, ":", shader_names[n_shaders], ":\n\n", processed_info_log);
    }
    else
    {
...
int end_program()
{
    ...
    if(error == 0)
    {
        ...
        *d++ = 0;
        ...
        log_warning("could not compile program ", program_name, "\n", processed_info_log);
    }
    return error;
}
...
void process_file(char* filename, int depth, char* parent_filename)
{
    ...
    if(!file)
    {
        log_output(parent_filename, ":0:0: error: could not open ", filename, ", error ", errno, "\n");
        return;
    }
    ...
    int ferror = fclose(file);
    if(ferror) log_warning("could not close file ", filename);
        ...
        if(*s == '#')
        {
...

            {
                ...
                if(strncmp(s+1, command, len(command)-1) == 0)
                {
                    ...
                    *(s++) = 0;
                    line_number++;
                    if(writing_shader) error = end_shader();
                    if(!error) break;
