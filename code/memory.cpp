...
{
    memory_stack* new_stack = (memory_stack*) malloc(size);
...
{
    memory_manager *manager = get_context()->manager;
    assert(manager->n_stallocs > 0 && memory == manager->stallocs[manager->n_stallocs-1].data, "attempted to stunalloc memory that is not stack allocated, or that is not on top of the stack");
